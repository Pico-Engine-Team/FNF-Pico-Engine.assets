...
function createSpeaker(attachedCharacter, offsetX, offsetY)
    ...
    runHaxeCode([[
        ...
        function startVisualizer() {
            var spectralAnalyzerClass = Type.resolveClass('funkin.vis.dsp.SpectralAnalyzer');
            if (spectralAnalyzerClass == null || FlxG.sound.music == null || FlxG.sound.music._channel == null) {
                visualizer = null;
                return;
            }

            visualizer = Type.createInstance(spectralAnalyzerClass, [FlxG.sound.music._channel.__audioSource, 7, 0.1, 40]);
            visualizer.fftN = 256;
        }
